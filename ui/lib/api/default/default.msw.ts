...
export const getGetConfigResponseEditorConfigMock = (
  overrideResponse: Partial<EditorConfig> = {},
): EditorConfig => ({
...
export const getPatchConfigResponseEditorConfigMock = (
  overrideResponse: Partial<EditorConfig> = {},
): EditorConfig => ({
  ...{ client: faker.string.alpha({ length: { min: 10, max: 20 } }) },
  ...overrideResponse,
...
export const getClearProjectCacheMockHandler = (
  overrideResponse?:
    ...
    | ((
        info: Parameters<Parameters<typeof http.post>[1]>[0],
      ) => Promise<ClearProjectCacheResponse> | ClearProjectCacheResponse),
  options?: RequestHandlerOptions,
) => {
  return http.post(
    '*/storage/project-cache/clear',
    async (info: Parameters<Parameters<typeof http.post>[1]>[0]) => {
      return HttpResponse.json(
        overrideResponse !== undefined
          ? typeof overrideResponse === 'function'
            ? await overrideResponse(info)
            : overrideResponse
          : getClearProjectCacheResponseMock(),
        { status: 200 },
      )
    },
    options,
